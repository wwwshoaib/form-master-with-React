...
const HookForm = () => {
    const emailState = useInputState('shoaibShort@gmail.com');
    const passwordState = useInputState('12345');
    const handleSubmit = e => {
        e.preventDefault();
        console.log('data from custom hook', emailState.value, passwordState.value)
    }
    return (
        <div className="flex flex-col items-center justify-center w-screen h-screen bg-gray-200 text-gray-700">
            ...
            <form className="flex flex-col bg-white rounded shadow-lg p-12 mt-12" onSubmit={handleSubmit}>
                ...
                <input className="flex items-center h-12 px-4 w-64 bg-gray-200 mt-2 rounded focus:outline-none focus:ring-2"        {...emailState} type="text" name="email" />
                <label className="font-semibold text-xs mt-3">Password</label>
                <input className="flex items-center h-12 px-4 w-64 bg-gray-200 mt-2 rounded focus:outline-none focus:ring-2"       {...passwordState} type="password" name="password" />
